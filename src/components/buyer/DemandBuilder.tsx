      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-stone-100 pb-4">
        <div>
          <h2 className="text-lg font-extrabold text-slate-900 flex items-center gap-2">
            <span>Post a New Demand</span>
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-800">
              Multi-Crop Capable
            </span>
          </h2>
          <p className="text-xs text-stone-500 mt-0.5">
            Algorithmic demand pooling discovers & aggregates nearby smallholder quantities
          </p>
        </div>
        <div className="text-right text-xs">
          <span className="text-stone-400 font-mono text-[11px]">Surat APMC Benchmark • </span>
          <span className="text-emerald-700 font-bold">Live Hourly Ticker Active</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="p-2.5 rounded-xl bg-stone-50 border border-stone-200 text-xs font-semibold text-slate-800 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-emerald-600" />
              <span>Tomorrow Morning Fresh Harvest (04:00 AM Dispatch)</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div>
            <label className="block font-bold text-slate-700 mb-1">
              Preferred Farm-Gate Pickup Window
            </label>
            <input
              type="text"
              value={pickupWindow}
              onChange={(e) => setPickupWindow(e.target.value)}
              className="w-full px-3 py-2 rounded-xl border border-stone-300 font-mono font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
            />
            <span className="text-[10px] text-stone-400 mt-0.5 block">Recommended 4:00 AM – 5:00 AM for perishable freshness</span>
          </div>
          <div>
            <label className="block font-bold text-slate-700 mb-1">
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-xl border border-stone-300 font-mono font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
            />
            <span className="text-[10px] text-stone-400 mt-0.5 block">Target delivery at Surat APMC Bulk Terminal</span>
          </div>
        </div>
